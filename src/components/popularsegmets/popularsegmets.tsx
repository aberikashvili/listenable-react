...
function PopularSegment() {
  ...
  async function fetchSongs() {
    try {
      ...
      const songData = data.tracks.track.map(
        (track: {
          name: any;
          listeners: string;
          image: { [x: string]: any }[];
        ...
        <ul className="song-list">
          {songs.map((song, index) => (
            <li key={index} className="song-item">
              ...
              <p className="song-retention">
                {" "}
                <span style={{ color: "grey" }}>5min retention: </span>{" "}
                {song.retentionAmount} %
              </p>
              <p className="retention-amount">
                {" "}
                <span style={{ color: "grey" }}>Retention rate:</span>{" "}
                {song.retentionRate} %
              </p>
              <p className="view-count">
                {" "}
                <span style={{ color: "grey" }}>View count: </span>{" "}
                {song.viewCount} K
              </p>
            </li>
          ))}
