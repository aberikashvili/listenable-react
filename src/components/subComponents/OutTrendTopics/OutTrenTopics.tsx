...
const OutTrenTopics = () => {
  const [outTrendTopics, setOutTrendTopics] = useState<PopularTopic[]>([]);

  const fetchData = async () => {
    try {
      const response = await axios.get("/TopicsData.json");
      setOutTrendTopics(response.data);
    } catch (error) {
  ...
  return (
    ...
    >
      <Typography sx={{ mt: 2, ml: 2, mb: 3 }}>Out of trend Topics</Typography>
      {outTrendTopics.slice(0, 4).map((topic) => (
